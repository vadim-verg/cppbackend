...
template <typename T>
void PrintVectorWithDot(std::ostream& out, const std::vector<T>& vector) {
    int i = 1;
    for (auto& value : vector) {
        out << i++ << ". " << value << std::endl;
    }
...
bool View::ShowAuthorBooks() const {
    try {
        if (auto author_id = SelectAuthor()) {
            PrintVectorWithoutDot(output_, GetAuthorBooks(*author_id));
        }
    } catch (const std::exception&) {
        // По ТЗ в случае ошибок или отмены ShowAuthorBooks ничего не выводит
    }
    return true;
}
...
std::vector<detail::BookInfo> View::GetBooks() const {
    ...
    for (const auto& book : use_cases_.GetBooks()) {
        dst_books.push_back({book.GetTitle(), book.GetPublicationYear()});
    }
    return dst_books;
}
