...
// Теперь функция возвращает структуру ParsedGameData
ParsedGameData LoadGame(const std::filesystem::path& json_path) {
    ...
    model::Game game;
    app::LootInfoProvider loot_info; // Создаем экземпляр внешнего провайдера лута
    ...
    // Читаем время бездействия пса до отправки на пенсию (в секундах)
