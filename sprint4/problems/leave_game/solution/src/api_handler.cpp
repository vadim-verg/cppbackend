...
http::response<http::string_body> ApiHandler::HandleGetRecords(const http::request<http::string_body>& req) const {
    ...
    int start = 0;
    int max_items = 100; //массив должен по умолчанию содержать не более 100 элементов
    ...
    if (max_items > 100) {
        return MakeErrorResponse(http::status::bad_request, "invalidArgument", "maxItems elements count limit is 100", version);
    }
