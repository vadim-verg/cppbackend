...
int main(int argc, const char* argv[]) {
    ...
    std::optional<model::Game> game_opt;

    try {
        // загрузка карты
        if (std::filesystem::exists(args_opt->config_file)) {
            game_opt = json_loader::LoadGame(args_opt->config_file);
        } else {
            // Если файла нет, создаем пустую заглушку игры.
            model::Game fallback_game;
            game_opt = std::move(fallback_game);
        }
    } catch (const std::exception& ex) {
        // Защита на случай непредвиденных исключений парсера
        model::Game fallback_game;
        game_opt = std::move(fallback_game);
    }

    // Извлекаем ссылку на игру
    model::Game& game = *game_opt;
    ...
    try {
        ...
        // Создаём обработчик HTTP-запросов и связываем его со статикой и API
        auto handler = std::make_shared<http_handler::RequestHandler>(
            game, std::filesystem::path(args_opt->www_root), app
        );
