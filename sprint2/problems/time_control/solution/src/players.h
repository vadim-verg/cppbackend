...
class PlayerManager {
public:
    // Создать нового игрока на карте с автоинкрементом ID
    ...
    // Получить ассоциативный массив всех игроков для фильтрации в ApiHandler
...
// Результат входа в игру (чистые данные бизнес-логики)
struct JoinGameResult {
    std::string token;
    uint32_t player_id;
};

// Фасад для управления бизнес-логикой приложения
class Application {
public:
    explicit Application(model::Game& game)
        : game_(game) {}

    // Публичный метод для продвижения времени (игровой цикл)
    void Tick(double delta_time_seconds);

    // Бизнес-логика входа: возвращает токен и ID или nullopt, если карта не найдена
    std::optional<JoinGameResult> JoinGame(const std::string& user_name, const std::string& map_id);

    // Бизнес-логика получения игроков: возвращает список или nullopt, если токен невалидный
...
private:
    ...
    // Приватные методы физического движка перемещения
