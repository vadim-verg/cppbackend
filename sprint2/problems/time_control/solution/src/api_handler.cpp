...
http::response<http::string_body> ApiHandler::HandleGameTick(const http::request<http::string_body>& req) const {
    ...
    // 1. Проверка метода (разрешен только POST)
    if (req.method() != http::verb::post) {
        auto res = MakeErrorResponse(http::status::method_not_allowed, "invalidMethod", "Only POST method is expected", version);
        res.set(http::field::allow, "POST");
        return res;
    ...
    // 3. Парсинг тела JSON и валидация поля timeDelta
    int64_t time_delta_ms = 0;
    try {
        auto json_doc = boost::json::parse(req.body());
        const auto& json_obj = json_doc.as_object();

        // Поле timeDelta обязательно должно быть числом
        time_delta_ms = json_obj.at("timeDelta").as_int64();
    } catch (...) {
        return MakeErrorResponse(http::status::bad_request, "invalidArgument", "Failed to parse tick request JSON", version);
    }

    // Если передан отрицательный или невалидный шаг времени — отдаем 400 Bad Request
    if (time_delta_ms <= 0) {
        return MakeErrorResponse(http::status::bad_request, "invalidArgument", "Invalid timeDelta value", version);
    }

    // 4. Переводим миллисекунды в секунды (double) и вызываем игровой цикл приложения
    double delta_time_seconds = static_cast<double>(time_delta_ms) / 1000.0;
    app_.Tick(delta_time_seconds);

    // 5. Успех -> Возвращаем пустой JSON-объект {} со статусом 200 OK
    http::response<http::string_body> res(http::status::ok, version);
    res.set(http::field::content_type, "application/json");
    res.set(http::field::cache_control, "no-cache");
    res.body() = "{}";
