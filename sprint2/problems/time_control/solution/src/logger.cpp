...
// Константа для передачи JSON-данных через атрибуты Boost.Log
...
void JsonFormatter(logging::record_view const& rec, logging::formatting_ostream& strm) {
    ...
    // ИСПРАВЛЕНИЕ: Извлекаем ptime по его строковому имени "TimeStamp"
    ...
    // data (извлекаем по строковому ключу AdditionalData)
...
void LogRequest(std::string_view ip, std::string_view uri, std::string_view method) {
    ...
    data["URI"] = std::string(uri); // Ключ по ТЗ строго капсом
...
void LogResponse(int64_t response_time_ms, int status_code, std::string_view content_type, bool has_content_type) {
    ...
    if (has_content_type) {
        ...
        data["content_type"] = nullptr; // Явный JSON null
    }
