...
// Вспомогательная функция для безопасного чтения double.
// Обрабатывает как вещественные (as_double), так и целые числа (as_int64)
double GetDoubleValue(const json::value& val) {
    ...
    return 1.0; // Значение по умолчанию на случай некорректного типа
}
...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    if (root_object.contains("maps")) {
        ...
        for (const auto& map_value : maps_array) {
            ...
            // Если поля нет, выставляем считанную ранее дефолтную скорость игры.
