...
private:
    ...
    auto MakeGameStateResponse(const boost::beast::http::request<Body, boost::beast::http::basic_fields<Allocator>>& req, const app::Application& app_facade) const {
        ...
        // 1. Ошибка формата Bearer / Отсутствие заголовка -> 401 invalidToken
        if (!token_opt) {
        ...
        // 2. Токен не найден в базе активных сессий -> 401 unknownToken
        if (!players_opt) {
        ...
        // 3. Успех -> Собираем JSON с вещественными координатами
        ...
        for (const auto& player : *players_opt) {
            ...
            dog_data["dir"] = "U"; // По ТЗ направление по умолчанию "U"
