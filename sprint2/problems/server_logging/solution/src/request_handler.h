...
public:
    ...
    void operator()(std::string ip, http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        // ЯВНО указываем std::string_view и конструируем из бустовских данных через .data() и .size()
        ...
        // Логируем запрос (теперь типы совпадают идеально)
        ...
        auto logging_send = [start_time, send = std::forward<Send>(send)](auto&& response) {
            ...
            // Исправляем тернарный оператор: приводим результат к std::string_view вручную
...
void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
    ...
    // Переводим decoded_target в std::string_view для точного сравнения
    ...
    if (target_sv.starts_with("/api/")) {
        ...
        } else if (std::string_view map_id = Endpoints::ExtractMapId(target_sv); !map_id.empty()) {
    ...
    // Проверяем безопасность пути (чтобы не вышли из корня статики)
    ...
    // Если всё успешно — отправляем файл симулятора в браузер!
